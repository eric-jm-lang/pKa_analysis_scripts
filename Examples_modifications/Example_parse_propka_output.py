...
def dictionnaries(Identifier, Chains, Resid_count, Chains_het, List, OutFile3):
    ...
    for l in list_results:
        ...
        if chain in Chains_het: # these chains are for the ligand can be removed if ligand not charged
            if resid == 'N': # here ligand is an amino acid, amine group
                Id_2_Numb[ID]= (len(Chains) - len(Chains_het))*Resid_count + (Chain_2_Numb[chain] - (len(Chains) - len(Chains_het)))
            if resid == 'C': # and this is the acid group. 
                Id_2_Numb[ID]= len(Identifier) + (Chain_2_Numb[chain] - (len(Chains) - len(Chains_het)))

    # Here we deal with a metal ion in the active site residue, remove if you don't have one, or modify with your ions, chain names and number
...
OutFile3 = open('residue_list.dat', 'w')
