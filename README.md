...
- Python 2
	the following python modules needs to be installed (e.g. using pip):
	* glob
	* numpy 
...
Here the HETATM entries correspond to the Mn2+ ions and the PHE ligand (which 
...
Zn2+ instead of Mn2+ it could have been [CBZN].
...
ligand ID (e.g. (len(Chains) - len(Chains_het))*Resid_count + (Chain_2_Numb
[chain] - (len(Chains) - len(Chains_het))) for 'N') you can always find out 
the number manually. 
...
at the end of 'Example_residues.dat' where PH1 corrsponds to the amine 
...
for example on line 123 of 'Example.pka' there is the following:
...
There are 4 Mn2+ in the structure file, to add them to the dictionnary 
